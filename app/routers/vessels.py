...
from ..dates import eta_sort_key
...
def vessel_files_page(
    ...
    for f in files:
        ...
        rows.append({
            ...
            "eta_sort": eta_sort_key(live_call.etb, live_call.eta) if live_call else eta_sort_key("", ""),
        })

    # ordenados por ETB (o ETA si no hay ETB) estimado -- Alongside/At roads
    # primero (ya llegaron), despues fechas ascendentes, sin nada al final
    rows.sort(key=lambda r: r["eta_sort"])
